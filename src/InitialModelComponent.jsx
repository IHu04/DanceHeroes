...
const InitialModelComponent = ({ isGameStarted, updateScore }) => {
  ...
  const [selectedModel, setSelectedModel] = useState('/models/shibu_sendagaya.vrm');
  ...
  const loadModel = (modelPath) => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        vrm.scene.position.set(-0.5, 0.5, 0);
  ...
  const onResults = async(results) => {
    ...
    function sendAllBonePositions(vrm) {
      ...
      axios.post('http://localhost:5001/user_bone_positions', bonePositions)
        .then((response) => {
          console.log('User positions sent.');
          // Request score calculation
          axios.get('http://localhost:5001/calculate_score')
            .then((response) => {
              console.log('Score calculated:', response.data.total_points);
              const pointsToAdd = response.data.total_points;
        ...
        .catch((error) => {
          console.error('Error sending user positions:', error);
        });
    }
  ...
  return (
    ...
    <div>
      ...
      <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 1 }}>
        <button onClick={() => setSelectedModel('/models/shibu_sendagaya.vrm')}>
          Model 1
        </button>
